...
class ScoreCalculator
{
    ...
    public function updateScore($frames)
    {
        ...
        foreach ($frames as $frameIndex => $frame) {
            ...
            $frameNextScore = isset($frames[$frameIndex + 1])
                ? $frames[$frameIndex + 1]->getScore()
                : null;
            ...
            if ($frameIndex < 9) {
                ...
                if ($frameScore["firstThrow"] === $totalPins) {
                    ...
                    if (
                        ...
                        $scoreTotal += $this->calcSpareBonus(
                            $frameIndex,
                            $frames
                        );
    ...
    {
        $scoreTotal = $frameScore["firstThrow"] + $frameScore["secondThrow"];
    ...
    private function calcStrikeBonus($frameIndex, $frames, $totalPins)
    {
        $frameNextScore = isset($frames[$frameIndex + 1])
            ? $frames[$frameIndex + 1]->getScore()
            : null;
        $frameNextNextScore = isset($frames[$frameIndex + 2])
            ? $frames[$frameIndex + 2]->getScore()
            : null;
    ...
    {
        $scoreTotal =
            $this->calcBaseFrameScore($frameScore) + $frameScore["thirdThrow"];
    ...
    private function handleFinalFrame($throwResult)
    {
        ...
        if ($this->currentThrow === 2) {
            ...
            if ($currentScores["firstThrow"] + $throwResult === 10) {
                ...
                $this->currentRemainingPins = 10;
                $this->currentThrow++;
            } elseif ($currentScores["firstThrow"] === 10) {
                // 1投目がストライクだった場合
                $this->currentThrow++;
            }
            return;
    ...
    private function checkGameOver()
    {
        ...
        if (
            ...
            if (
                ...
                $lastFrameScore["firstThrow"] + $lastFrameScore["secondThrow"] <
                    10
            ) {
